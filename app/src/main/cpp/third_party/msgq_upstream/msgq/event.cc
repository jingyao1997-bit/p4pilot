...
#ifdef __ANDROID__
  const char *p4pilot_ipc_dir =
      std::getenv("P4PILOT_IPC_DIR");

  if (p4pilot_ipc_dir == nullptr ||
      p4pilot_ipc_dir[0] == 0) {
    throw std::runtime_error(
        "P4PILOT_IPC_DIR is not configured"
    );
  }

  std::string shm_path =
      std::string(p4pilot_ipc_dir) + name;
